...
void setup()
{
  ...
  SPI_3.begin();
  SPI_3.beginTransaction(myMT6835SPISettings); // Mode 3
  Enc1_SPI.init(&SPI_3);
...
bool zeroMT6835()
{
  ...
  // Send the combined 16-bit value as two 8-bit parts
  SPI_3.transfer((command >> 8) & 0xFF); // Send the upper 8 bits (command + upper part of address)
  SPI_3.transfer(command & 0xFF);        // Send the lower 8 bits (lower part of address)
